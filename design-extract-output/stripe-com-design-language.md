...
> Extracted from `https://stripe.com` on April 16, 2026
> 2410 elements analyzed
...
|------|-----|-----|-----|-------------|
| Primary | `#533afd` | rgb(83, 58, 253) | hsl(248, 98%, 61%) | 900 |
| Secondary | `#50617a` | rgb(80, 97, 122) | hsl(216, 21%, 40%) | 320 |
| Accent | `#e8e9ff` | rgb(232, 233, 255) | hsl(237, 100%, 95%) | 58 |

### Neutral Colors

| Hex | HSL | Usage Count |
|-----|-----|-------------|
| `#000000` | hsl(0, 0%, 0%) | 2626 |
| `#ffffff` | hsl(0, 0%, 100%) | 186 |
| `#101010` | hsl(0, 0%, 6%) | 6 |

### Background Colors

Used on large-area elements: `#ffffff`, `#e5edf5`, `#f8fafd`, `#0d1738`, `#322d27`, `#e6ebf1`
...
```css
background-image: linear-gradient(90deg, rgba(0, 0, 0, 0) 0px, rgb(196, 47, 165) 50%, rgba(0, 0, 0, 0) 100%);
```
...
| Hex | Contexts | Count |
|-----|----------|-------|
| `#000000` | text, border, background | 2626 |
| `#533afd` | text, border, background | 900 |
| `#50617a` | text, border | 320 |
| `#061b31` | text, border, background | 314 |
| `#ffffff` | background, text, border | 186 |
| `#64748d` | text, border | 172 |
| `#0000ee` | text, border | 90 |
| `#e8e9ff` | background, border | 58 |
...
- **sohne-var** — used for all (2410 elements)
...
| 15px | 0.9375rem | 400 | normal | normal | div |
...
--hds-color-core-brandDark-600-a70: #533afdb2;
...
**Durations:** `0.24s`, `0.3s`, `0.25s`, `0.15s`, `0.1s`, `0.8s`, `0.6s`, `1s`, `0.5s`, `0.4s`, `1.2s`, `0s`, `0.12s`, `0.2s`
...
**52 grid containers** and **371 flex containers** detected.
...
|----------|-----------|-------------|-------------|-----------|-------------|
| mobile (375px) | 16px | Yes | 12 | Yes | 19487px |
| tablet (768px) | 16px | Yes | 12 | Yes | 14279px |
| desktop (1280px) | 16px | Yes | 12 | No | 14418px |
| wide (1920px) | 16px | Yes | 12 | No | 14666px |

### Breakpoint Changes

**375px → 768px** (mobile → tablet):
- H1 size: `34px` → `40px`
- Page height: `19487px` → `14279px`
...
```css
/* Focus */
outline: rgb(6, 27, 49) none 3px → rgb(83, 58, 253) solid 2px;
```

**"Solutions"**
```css
/* Hover */
color: rgb(82, 99, 123) → rgb(28, 48, 71);
border-color: rgb(82, 99, 123) → rgb(28, 48, 71);
outline: rgb(82, 99, 123) none 3px → rgb(28, 48, 71) none 3px;
```
```css
/* Focus */
color: rgb(82, 99, 123) → rgb(7, 28, 50);
border-color: rgb(82, 99, 123) → rgb(7, 28, 50);
outline: rgb(82, 99, 123) none 3px → rgb(83, 58, 253) solid 2px;
```

**"Developers"**
```css
/* Hover */
color: rgb(100, 116, 141) → rgb(40, 59, 82);
border-color: rgb(100, 116, 141) → rgb(40, 59, 82);
outline: rgb(100, 116, 141) none 3px → rgb(40, 59, 82) none 3px;
```
```css
/* Focus */
color: rgb(100, 116, 141) → rgb(7, 28, 50);
border-color: rgb(100, 116, 141) → rgb(7, 28, 50);
outline: rgb(100, 116, 141) none 3px → rgb(83, 58, 253) solid 2px;
```
...
border-color: rgb(83, 58, 253) → rgb(46, 43, 140);
opacity: 1 → 0.897168;
...
**Overall Score: 94%** — 111 passing, 7 failing color pairs
...
| `#ff6118` | `#ffe0d1` | 2.41:1 | FAIL | div (2x) |
| `#000000` | `#0d1738` | 1.2:1 | FAIL | section (1x) |
| `#000000` | `#322d27` | 1.54:1 | FAIL | div (1x) |

### Passing Color Pairs

| Foreground | Background | Ratio | Level |
|------------|------------|-------|-------|
| `#000000` | `#7f7dfc` | 6.24:1 | AA |
| `#000000` | `#ffffff` | 21:1 | AAA |
| `#000000` | `#e5edf5` | 17.76:1 | AAA |
| `#ffffff` | `#533afd` | 6.19:1 | AA |
| `#000000` | `#f8fafd` | 20.08:1 | AAA |
| `#000000` | `#e8e9ff` | 17.53:1 | AAA |
| `#64748d` | `#ffffff` | 4.75:1 | AA |
| `#64748d` | `#f8fafd` | 4.54:1 | AA |
| `#533afd` | `#e2e4ff` | 4.95:1 | AA |
...
**Strengths:** Consistent typography system, Well-defined spacing scale, Consistent border radii, Strong accessibility compliance, Good CSS variable tokenization

**Issues:**
- 14 distinct font sizes — consider a tighter type scale
...
|-------|-------|----------|
| modal | 2147483639,2147483639 | iframe |
| sticky | 99,99 | div.U.n.i.v.e.r.s.a.l.C.h.a.t.I.n.i.t.i.a.l.i.z.e.r._._.c.t.a |
...
**79 unique SVG icons** detected. Dominant style: **filled**.
...
**Aspect ratios:** 4.18:1 (36x), 1:1 (10x), 3:4 (8x), 21:9 (8x), 9:16 (4x), 4:3 (2x), 16:9 (2x), 3:2 (1x)
