# ASG TV Remote (Raspberry Pi kiosk)

A dependency-free Node static server and phone/tablet remote that drives the
office TV dashboard on a Raspberry Pi. It lives at `infra/pi-remote/` and serves
the whole repo workspace over HTTP on the local network, so it can open
dashboard pages from `apps/admin-hub/`.

> Commands below are run from this directory (`infra/pi-remote/`).

## Project Structure

- `index.html` - remote control UI for phone/tablet
- `server.js` - Node server and remote control API (serves the repo root)
- `previews/` - dashboard preview images served by the remote
- `scripts/start-dashboard.sh` - kiosk launcher for fullscreen dashboard
- `scripts/deploy-pi.sh` - rsync/SSH deploy to the Pi
- `systemd/` - ready-to-use service files for boot automation
- dashboard pages come from `apps/admin-hub/components/` (one level up in the repo)
